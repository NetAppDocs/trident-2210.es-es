...
En el archivo de definición de backend de ejemplo que se muestra a continuación, se establecen valores predeterminados específicos para todos los grupos de almacenamiento, que establecen el `type` En Silver. Los pools de almacenamiento virtual se definen en la `storage` sección. En este ejemplo, algunos del pool de almacenamiento establecen su propio tipo, y algunos pools sobrescriben los valores predeterminados establecidos anteriormente.
