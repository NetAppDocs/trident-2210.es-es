...
* Añadido link:https://docs.netapp.com/us-en/trident/trident-reco/security-reco.html#enable-per-volume-host-side-encryption-using-linux-unified-key-setup-luks["Compatibilidad con volúmenes cifrados LUKS"] para `ontap-san` y.. `ontap-san-economy` impulsores del almacenamiento.
